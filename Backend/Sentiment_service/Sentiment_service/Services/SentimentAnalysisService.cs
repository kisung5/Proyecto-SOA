...
namespace Sentiment_service.Services
{
    class SentimentAnalysisService
    {
        ...
        private async void ProcessMessage(string message)
        {
            ...
            float result = _NLP.sentiment_analysis(request.FileName);

            // 3) Creates new object
            var analysisResult = new
            {
                FileName = request.FileName,
                SentimentPercentage = result
            };
            ...
            Console.WriteLine(string.Format("File {0} have a sentiment percentage of {1}", request.FileName, result));
        }
