...
const API_BASE = 'https://localhost:5001/swagger/'
...
export const logoutHandler = async (token) => {
  ...
  await axios.post(API_BASE+'/logout',postBody)
}
...
export const requestEditLock = async (token, formId) => {
  ...
  const resp = await axios.post(API_BASE+'/lock', postBody)
  return resp
}
...
export const sendUnlockMessage = async (token, formId) => {
  ...
  const resp = await axios.post(API_BASE+'/unlock', postBody)
  return resp
}